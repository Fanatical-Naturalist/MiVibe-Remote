...
- 托盘加入当前登录会话的命名互斥体。隐藏烟雾测试连续启动两个 0.2 实例：首个按 8 秒测试周期正常退出，第二个在 3 秒内以退出码 0 返回；测试前后 Windows 启动值保持不变，开发构建残留托盘进程为 0。
- 主动“连接/重新连接”改为先请求旧 resident 完成安全停止，只有确认旧进程退出后才建立新会话。10 秒清理超时时不会强制终止，也不会启动第二个桥；同时修复“安全退出超时后托盘先消失、桥仍遗留”的生命周期缺口。
- 开机自动启动采用 `HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run`，默认关闭；关闭时仅在值与当前可执行文件绝对路径完全一致时删除，避免覆盖或清理用户的无关启动项。
- GATT resident 加入标准 `0x180F / 0x2A19` 电量监视：首次 Uncached 读取，Notify 可用时订阅，否则每 300 秒轮询；严格接受单字节 `0–100`，失败只报告“未知/上次读取”，不改变语音桥退出码。
- 电量跨进程使用版本化 ASCII 行协议 `MIVIBE_EVENT|1|BATTERY|...`。托盘拒绝畸形、未知版本和旧子进程延迟输出，避免断线重连后由旧缓冲数据覆盖新会话状态。
- 电量初始化改为后台执行：最坏情况下的 GATT 超时不会阻塞 Typeless/Codex Voice 桥先行启动；退出时先取消并等待初始化/轮询收束，再清除通知订阅。
- 双击托盘现在打开 0.2 极简状态窗。窗口采用石墨/银色和冷蓝主操作色，跟随 Windows 深浅色与高对比模式，展示简介、设备、连接、电量、操作按钮和三条按键提示；Esc、关闭按钮和 Alt+F4 只隐藏回托盘。
...
## 2026-09-05：原始 HID 旁路与新按键映射实测

- 用户提供 RC003 WUDFHost/Frida 方案，并授权在电脑前做限时诊断。
- 确认硬件族与参考实现一致，使用独立脚本而非替换现有安装版；
  两轮原始观察及一轮动作测试均完成主动卸载/断开，无目标宿主崩溃记录。
- 首轮 37 次调用因过严的 Information 长度检查被过滤；第二轮调整后收到
  22 个有效九字节报告，同流识别返回、音量加减和 TV；本机 Information 实际为 0。
- 独立菜单钩子发送右 Shift＋T，保持约 80 ms，用户确认打开 Typeless Translate。
- 同流三键校准后的动作测试发送两次扩展 Delete、前后各一次 Codex 相邻任务导航；
  用户确认 Delete 短按/长按及音量上下切换均正常。
- 19:39:31 左右动作测试自动结束，发送器关闭、脚本卸载、Frida 主动 detach
  均报告成功；菜单临时钩子更早已正常解除，未残留按住的注入键。
- 源码菜单映射与界面说明调整为 Translate；原始 HID 返回/音量仍是独立实验工具，
  待完成管理员启动、重连及稳定性设计后再接入常驻程序。
- 详细边界与用户新映射决策见 D-041 和 `docs/HID_DIAGNOSTIC_2026-09-05.md`；
  原始日志、设备唯一标识、依赖和构建缓存均留在本地忽略目录。

## 2026-09-05：0.3 常驻集成与前端调整

- 用户要求将已测的新键位纳入 0.3：返回单次 Delete、菜单 Typeless Translate、音量加减按 Codex 侧栏顺序切换相邻对话。Home 保留 Delete，TV 仍为原始反引号。
- 0.3 增加独立管理员增强按键组件及托盘生命周期接线：主动启用、三键校准、状态回传、断线恢复后重校准、停用与安全退出。
- 每次应用启动均需手动启用增强按键并响应 Windows 管理员提示；登录自动启动不自动启用增强组件。
- 前端沿用实拍校准遥控器图及深绿/暖橙配色，增加 0.3 标识和增强按键控制卡，保留浅色/高对比模式；新键位说明由实际增强状态驱动。
- UI 与状态契约构建通过，0 个警告、0 个错误。渲染检查与完整候选包的实机验收继续进行，未将之前临时探针成功写成新包已经验收。
- 同步 0.3 使用文档和发布说明，保留 0.2 发布记录及此前诊断日志作为历史证据。

### 0.3 本地交付构建结果

- 完整 Release 构建与安装包/便携 ZIP 生成通过。323 个 payload 文件校验一致；没有覆盖当前安装目录或创建公开 Release。
- Python 19 项自动测试、JavaScript 解析/租约模拟检查、最终程序集 4 条合法与 15 条畸形协议消息检查通过。
- 打包后的增强组件实机观察测试通过：普通权限明确拒绝附加，管理员运行进入校准状态，10 秒收到 6 个心跳，零按键动作，安全退出码 0。
- 修复当前 144 DPI 下控制中心标题及状态说明裁切，检查明暗主题与已启用/校准/未启用六种视图。
- 该次构建完成时，旧版仍在安装目录运行；当时已请求用户通过安全退出释放旧实例，新包整机按键与语音验收尚待执行。详情见 VALIDATION_0.3.0-alpha.1.md。

### 0.3 主程序启动与 alpha.2 暂停修复

- 用户确认旧版已安全退出，进程检查无旧版残留后启动alpha.1便携版。语音桥armed，真实电量61%。用户完成增强按键三步校准，界面与管道状态均显示已启用。
- 该轮返回/Delete、音量侧栏切换、菜单翻译和语音的整机反馈当时仍待用户回复，alpha.1 实例在等待期间保持运行；后续反馈见活动视图导航修正记录。
- 复核修复了暂停等待期间仍处理基础键、暂停后重装钩子、故障控制器重连不重建三处边界，并单独打包alpha.2。构建、六种UI控制状态、19条协议消息检查通过，324个payload校验通过。
- 该次 alpha.2 文档与中文 README 已同步到候选包；未交付草稿仅在项目内归档，打包时未改动当时运行中的 alpha.1 或安装目录。后续证据见 VALIDATION_0.3.0-alpha.2.md。

### 2026-09-06：活动视图导航修正

- 用户确认 alpha.1 返回/Delete 与菜单翻译基本正常；音量键在聚焦 Codex 输入框后可以切换，但顺序与活动视图显示不一致，要求改为按眼前列表上下切换。
- 只读核对本机 Codex 26.901.5280.0：默认任务切换组合同时用于面板标签；活动视图的默认导航数据还可能仅包含 priority 分组，和界面中的完整活动列表不一致。独立快捷键只能解决面板冲突，因此未采用，也未修改 Codex 设置。
- alpha.2 改为通过 Windows 可访问性接口选择活动列表的相邻已加载任务行；菜单展开、当前行不明确、列表变化或焦点改变时跳过。普通视图保留原快捷键，改为完整四事件单批发送并按已接受前缀清理。
- 普通快捷键离线回归 27 项通过，零真实原生输入；活动视图新行为仍需实际按键验收。该轮修订时保留了正在运行的 alpha.1，仅修改尚未交付的 alpha.2 候选；实际切换见下方续记。

### 2026-09-06 00:24–00:25：最终 alpha.2 启动与实机测试

- 生产实现的只读 `CaptureSnapshot` 检查成功：已加载 `20` 行、当前索引 `1`、上下均可导航，耗时 `293 ms`。该检查没有切换任务，仅确认当前活动列表可识别。
- 通过旧 alpha.1 自身的“安全退出”结束程序，并确认 Tray、GattProbe、KeyBridge 均无残留，随后启动最终 alpha.2 便携包；新 Tray PID 为 `34016`。安装目录和 alpha.1 产物保留不变。
- 00:25，UI Automation 确认新程序“已连接”、真实电量 `60%`、“增强已启用”；用户已完成三键校准，辅助组件管道身份认证成功，语音桥报告 `armed`。
- 该时段用户开始测试音量－按活动列表下移、音量＋回到原任务，并回归返回/Delete 与菜单/Translate；当时结果待反馈，未将只读识别或成功启动写成实体导航验收通过。
- 最终构建检查：27 项普通导航离线回归、19 条协议消息、6 种 UI 控制状态检查通过；367 个 payload 文件及 2 个发布文件 SHA-256 全部匹配。此前 Python 19 项测试为另一组辅助组件证据，分别保留。
- 此时运行实例已切换为最终 alpha.2；随后收到的活动列表与按键反馈见下节，完整范围见 VALIDATION_0.3.0-alpha.2.md。

### 2026-09-06：最终 alpha.2 活动列表与按键回归通过

- 用户确认音量－按当前活动视图下移一项，音量＋回到原任务，列表顺序正确。
- 用户确认新版返回/Delete、菜单/Typeless Translate 两项均正常。
- alpha.2 日志同时记录 Activity 上一/下一任务、返回 Delete、菜单 Typeless 翻译动作，与用户反馈一致。
- 当前已通过活动列表相邻任务往返导航及返回/翻译入口回归。语音桥 `armed` 仅表示连接就绪；语音实际输入、完整翻译输出、暂停恢复、安装升级、登录启动、驱动断线重连和卸载仍保留为未完成范围。
- 此次只更新文档，不修改构建产物，367 个 payload 文件与 2 个发布文件的既有校验结果保持不变。

### 2026-09-06：alpha.3 安装、开始菜单与 Backspace 验收

- 根据用户反馈将返回键改为 Backspace，Home 保留 Delete；界面和中英文说明同步。TV 保留原始反引号，单独重映射仍需处理其原生输入抑制。
- 正式安装目录升级至 alpha.3，Setup 成功且无需重启，系统扫描码映射未变；用户确认返回/Home 删除方向与长按单次行为均正确。
- 完成开始菜单固定及登录自动启动；重复点击应用入口可显示现有窗口，没有第二个常驻实例。
- 368 个正式安装文件与发行包一致，三个 GitHub 附件摘要核对通过。公开发布使用源码提交 388e3fa；详细验收见 VALIDATION_0.3.0-alpha.3.md。