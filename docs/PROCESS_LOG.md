...
- 用户确认希望托盘具备简介和稍微精致的极简前端，而不仅是一组右键菜单。
- 确定两层交互：右键托盘承担快捷动作；双击托盘打开小型状态窗口，关闭窗口只隐藏回托盘。
- 窗口只展示一句简介、设备与连接状态、电量、重新连接、暂停/启动、音频检查、开机启动和三条按键提示。
- 视觉使用石墨灰/银色与单一冷蓝强调色，跟随 Windows 深浅色模式；完整遥控器图和映射编辑器继续排除在 0.2 之外。
