...
2. TV 实体键一键发送 Codex Voice 快捷键（当前用户配置为 `Ctrl + \``）；菜单键保留给后续动作。
3. 使用方向环、确认键以及其他可用实体键完成高频导航和编辑。
...
| 确认 | `Enter` | 可配置 | 双击默认禁用 |
| 返回 | `Esc` | `Alt + Left` | 当前 Windows 用户态无法收到该实体键事件 |
| Home | `Backspace` | 暂不配置 | 已确认；不再用于打开或聚焦 Codex |
| 音量 + / - | 系统音量 | 连续调节 | 当前 Windows 用户态无法收到实体键事件 |
| 菜单 | 待确认 | 对话/项目切换候选 | 暂不占用 |
| TV | Codex Voice：`Ctrl + \`` | 待确认 | v0.1 已采用；运行期间电脑反引号同样触发 Voice，退出后恢复 |

真实映射以按键探针采集的数据为准，不根据遥控器图标猜键码。
...
常驻语音核心已完成第一版：`--resident` 无固定时限运行，流式音频不在内存中无限累积，`Ctrl+C` 会关闭 ATVV、取消订阅并恢复 TV/反引号钩子。下一步把该核心承载到托盘应用，增加暂停、退出、断线重连与状态反馈。

托盘 MVP 第一版已实现：自动启动隐藏语音桥、状态菜单、暂停/恢复、音频路由体检和安全退出。托盘通过 Windows 命名事件请求子进程正常清理，不使用强制结束；两轮自动测试均无残留进程。`0.2.0-alpha.1` 已加入 GATT 断线检测、2/5/10/30 秒封顶退避重连与最近 10 份托盘日志保留；自动调度测试通过，等待实体断开/恢复验收。开机启动仍待实现。
...
首次用户验收已通过：路由弹窗中文正常、CABLE/AirPods 三项检查通过，TV 键从托盘后台桥成功打开 Codex Voice。
...
验收：连续使用 30 分钟无粘键；断连重连后自动恢复；除已明确接受的 TV/反引号 v0.1 限制外，不拦截普通键盘；退出后反引号必须立即恢复。
...
0.2 首次只做轻量状态窗口，范围控制为：一张遥控器设备卡、连接/重连状态、电量百分比、启动/暂停和音频路由体检。它复用现有 WinForms 托盘，不在此阶段引入新的前端框架，也不阻塞蓝牙和安装包。完整遥控器示意图与可视化映射器保留为后续增强。
...
当前状态：Typeless 分离式语音输入和 TV 控制的 Codex Voice 均已通过实体端到端验收；下一步将探针能力收敛进后台常驻 MVP。
...
- 若日常使用证明反引号占用不可接受，为 TV 键增加设备级无损区分；正式发布需纳入驱动签名、安装/卸载回退和 Windows 版本兼容测试。
